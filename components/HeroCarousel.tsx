...
export default function HeroCarousel({ onScrollToContact }: { onScrollToContact: (e: React.MouseEvent<HTMLAnchorElement>) => void }) {
  ...
  return (
    <div className="relative max-w-7xl mx-auto min-h-[460px] flex flex-col justify-between">
      {/* Content wrapper with fade transition */}
      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center transition-all duration-500 ease-in-out transform ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        ...
        <div className="lg:col-span-7 text-left flex flex-col justify-center">
          <div>
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 dark:text-white tracking-tight mb-6 leading-tight">
              {currentSlide === 0 ? (
                <>
            ...
            <p className="text-base sm:text-lg text-gray-600 dark:text-zinc-400 leading-relaxed mb-8 max-w-2xl">
              {slide.subheadline}
            </p>
            ...
            <div className="flex flex-col sm:flex-row justify-start items-center gap-4">
              <a
                href="https://wa.me/096788208"
                target="_blank"
                rel="noopener noreferrer"
                className="w-full sm:w-auto px-8 py-4 rounded-lg bg-accent-blue hover:bg-accent-blue-hover text-white font-semibold transition-all duration-200 shadow-lg shadow-blue-500/10 hover:shadow-blue-500/25 flex items-center justify-center space-x-2 text-base cursor-pointer"
              >
        ...
        <div className="lg:col-span-5 relative w-full h-[320px] sm:h-[420px] rounded-2xl overflow-hidden border border-gray-200 dark:border-zinc-800 shadow-xl bg-zinc-900">
          <Image
            src={slide.image}
            alt={slide.badge}
            fill
